...
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
Cnvrt_and_cmp	proc,				; Turn string into Integer, then compare
				string1: PTR BYTE,	; source string not modified
				string2: PTR BYTE	; source string not modified
.data
	intpass1	dd	?				; decimal holder for 1st string
	intpass2	dd	?				; decimal holder for 2nd string
	returnValu	dd	1				; value to hold return value
.code

	pushad							; save registers
	mov	returnValu, 1				; initialize return value to be held in eax after compare
	mov	intpass1, 0					; intialize int holder for string1
	mov	intpass2, 0					; intialize int holder for string2
	mov 	edx, string1			; our string
...
TOP_ONE:
	...
	jmp 	TOP_ONE			; until done

DONE_ONE:

	mov	intpass1, eax		; put result into this integer

	xor	edx, edx		; intialize edx
	mov 	edx, string2		; our string
	xor 	eax, eax 		; zero a "result so far"
...
TOP_TWO:
	...
	imul 	eax, 10 		; multiply "result so far" by ten
	add 	eax, ecx 		; add in current digit
	jmp 	TOP_TWO 			; until done

DONE_TWO:

	mov	intpass2, eax		; put result into this integer

COMPARE_TIME:
	cmp	intpass1, eax		; intpass1 bigger than eax?
	jg	INTONEBIGGER		; if so, return a value of 1 in eax
INTONELESSER:
	mov	returnValu, 0		; move 0 into eax, intpass1 is less than intpass2
INTONEBIGGER:
